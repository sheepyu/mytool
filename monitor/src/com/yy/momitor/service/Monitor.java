...
public class Monitor implements Runnable {
	...
	public Monitor(SqlSession session) {
		...
		try {
			...
			if(confElement.elementText("sleep") != null){
				sleepSeconds = Integer.valueOf(confElement.elementText("sleep"));
			}
	...
	public void monitorTD() {
		session.clearCache();
		List<TD> tds = session.selectList("com.yy.momitor.mapper.tdMapper.getTs");
		log.info(tds);
		for (TD td : tds) {
	...
	public void monitorZJ(){
		...
		session.clearCache();
		List<DL> dls = session.selectList("com.yy.momitor.mapper.dlMapper.getZj",ids);
		log.info(dls);
		String content ="";
		String title = "";
		for(DL dl : dlMonitor){
			for(int i= 0 ;i<dls.size();i++){
				if(dl.getDlid()==dls.get(i).getDlid()&&dl.getZjye()>dls.get(i).getZjye()){
